...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Uses EL for cloned voice quality + a real estate background image.
    // Faster render than Video Agent (~5–8 min vs 10+ min).
    if (profile.voice_clone_id && listingPhotos.length === 0) {
      console.log(`[create-blog] EL voice path — cloning TTS for voice ${profile.voice_clone_id}`);

      // Pick a real estate background based on whether it's short-form or landscape
      const defaultBg = isShortForm
        ? "https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=720&q=80"
        : "https://images.unsplash.com/photo-1570129477492-45c003edd2be?auto=format&fit=crop&w=1280&q=80";
      ...
      // EL failed — fall through to Video Agent below
    }
