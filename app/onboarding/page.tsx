        ...
        <div className="flex justify-center gap-4 mb-8">
          {stepMeta.map(({ n, label }) => (
            <div key={n} className="flex flex-col items-center gap-1.5">
              ...
              <span className={`text-xs font-medium ${step === n ? "text-primary-500" : "text-slate-400"}`}>{label}</span>
            </div>
          ))}
        </div>
            ...
            <div className="mb-6">
              ...
              {avatarPreview ? (
                ...
                <button onClick={() => avatarInputRef.current?.click()} disabled={uploadingAvatar}
                  ...
                  <p className="text-sm font-medium text-slate-600">{uploadingAvatar ? "Uploading…" : "Click to upload headshot"}</p>
              ...
              {logoPreview ? (
                ...
                <button onClick={() => logoInputRef.current?.click()} disabled={uploadingLogo}
                  ...
                  <div className="text-left">
                    <p className="text-sm font-medium text-slate-600">{uploadingLogo ? "Uploading…" : "Click to upload brokerage logo"}</p>
